...
/**
 * List all tip from nagios plugins directory
 *
 * @return array
 */
function get_parameters_tip()
{
    $parameters = array();
    $filepath = Config::get('nagios_plugins').'/param_tip/';
    foreach (scandir($filepath) as $filename) {
        if (pathinfo($filename, PATHINFO_EXTENSION) === 'txt') {
            $fileData = file_get_contents($filepath.$filename);
            $fileData = str_replace(array("\r\n", "\r", "\n"), '<br>', $fileData);
            $fileData = str_replace("'", '&#39', $fileData);
            $fileData = str_replace('"', '&#34', $fileData);
            $parameters[pathinfo($filename, PATHINFO_FILENAME)] = $fileData;
        }
    }
    return $parameters;
}
...
/**
 * Get absolute path for shell_exec
 *
 * @return string
 */
function get_nagios_plugins_path()
{
    return Config::get('nagios_plugins');
}
