...
function delete_device($id)
{
    ...
    log_event("Device $host has been removed", 0, 'device', 3, '1234');
 ...
 */
function addHost($host, $snmp_version = '', $port = '161', $transport = 'udp', $poller_group = '0', $force_add = false, $port_assoc_mode = 'ifIndex', $additional = array(), $user = null)
{
    ...
    if (isset($additional['snmp_disable']) && $additional['snmp_disable'] == 1) {
        return createHost($host, '', $snmp_version, $port, $transport, array(), $poller_group, 1, true, $additional, $user);
    }
        ...
        if ($snmpver === "v3") {
            ...
            foreach (Config::get('snmp.v3') as $v3) {
                ...
                if ($force_add === true || isSNMPable($device)) {
                    return createHost($host, null, $snmpver, $port, $transport, $v3, $poller_group, $port_assoc_mode, $force_add, array(), $user);
                } else {
            ...
            foreach (Config::get('snmp.community') as $community) {
                ...
                if ($force_add === true || isSNMPable($device)) {
                    return createHost($host, $community, $snmpver, $port, $transport, array(), $poller_group, $port_assoc_mode, $force_add, array(), $user);
                } else {
    ...
    if (isset($additional['ping_fallback']) && $additional['ping_fallback'] == 1) {
        ...
        $additional['os'] = "ping";
        return createHost($host, '', $snmp_version, $port, $transport, array(), $poller_group, 1, true, $additional, array(), $user);
    }
...
function createHost(
    ...
    if ($device_id) {
        log_event("Device $host has been added", 0, 'device', 3, null, $user);
        return $device_id;
    }
 ...
 */
function log_event($text, $device = null, $type = null, $severity = 2, $reference = null, $user = null)
{
    if (!is_array($device)) {
...
function add_schedule($cidr, $snmp_version, $community)
{
