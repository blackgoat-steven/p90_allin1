...
if ($port_details) {
    foreach (dbFetchRows('SELECT * FROM `ipv4_addresses` WHERE `port_id` = ?', array($port['port_id'])) as $ip) {
        ...
        echo "<span class=interface-desc>" . $ip['ipv4_address'] . '/' . $ip['ipv4_prefixlen'] . '</span>';
        $break = '<br />';
    }
...
if ($port['ifOperStatus'] == 'up') {
    ...
    echo "<i class='fa fa-long-arrow-left fa-lg' style='color:green' aria-hidden='true'></i>IN <span style='color: ".percent_colour($in_perc)."'>".formatRates($port['in_rate'])."<br />
        <i class='fa fa-long-arrow-right fa-lg' style='color:blue' aria-hidden='true'></i>OUT <span style='color: ".percent_colour($out_perc)."'>".formatRates($port['out_rate'])."<br />
        <i class='fa fa-long-arrow-left fa-lg' style='color:purple' aria-hidden='true'></i>INUcast ".format_bi($port['ifInUcastPkts_rate'])."pps</span><br />
        <i class='fa fa-long-arrow-right fa-lg' style='color:darkorange' aria-hidden='true'></i>OUTUcast ".format_bi($port['ifOutUcastPkts_rate']).'pps</span>';
}
...
if ($port_details && Config::get('enable_port_relationship') === true && port_permitted($port['port_id'], $device['device_id'])) {
    ...
    foreach (dbFetchRows('SELECT * FROM `ports` WHERE `pagpGroupIfIndex` = ? and `device_id` = ?', array($port['ifIndex'], $device['device_id'])) as $member) {
        ...
        echo "$br<i class='fa fa-cube fa-lg icon-theme' aria-hidden='true'></i> <strong>aaaaaaaaaaaaaaaaa".generate_port_link($member).' (PAgP)</strong>';
        $br = '<br />';
    }
