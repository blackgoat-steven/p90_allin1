...
';
    ...
    echo '<i class="fa fa-microchip fa-lg icon-theme" aria-hidden="true"></i> <strong>Processors</strong>';
    echo '</div>
    ...
    if (\LibreNMS\Config::get('cpu_details_overview') === false) {
        if ($screen_width = Session::get('screen_width')) {
            if ($screen_width > 970) {
                ...
                $graph_array['height'] = round($graph_array['width'] /1.5);
        ...
        //echo $db_name;
        //echo '/grafana/dashboard-solo/script/device_processor.js??orgId=1&panelId=1&hostname=' . $device['hostname'] . '&dbname=' . $db_name .'&theme=light" height="' . $graph_array['height'];
        echo ' <iframe src="/grafana/dashboard-solo/script/device_processor.js??orgId=1&panelId=1&hostname=' . $device['hostname'] . '&dbname=' . $db_name .'&theme=light" height="' . $graph_array['height'] . '" frameborder="0"></iframe>';
/*
...
*/
        ...
        foreach ($total_percent as $type => $values) {
            ...
            echo '
              <tr>
                <td class="col-md-4">' . $values['descr'] . '</td>
                <td class="col-md-4">' . 'x' . $values['count'].'</td>
                <td class="col-md-4">' . print_percentage_bar(200, 20, $percent_usage, null, 'ffffff', $background['left'], $percent_usage.'%', 'ffffff', $background['right']) . '</td>
                <!--
