...
if (!Auth::user()->hasGlobalAdmin()) {
    ...
    $paramtip = json_encode(get_parameters_tip());
    $shellpath = get_nagios_plugins_path();

    foreach (dbFetchRows('SELECT * FROM `devices` ORDER BY `hostname`') as $device) {
        $devicesform .= "<option value='".$device['device_id']."'>".$device['hostname']."/".format_hostname($device).'</option>';
    }
