...
/*
    ...
    foreach ($thumb_array as $period => $text) {
        ...
        echo('1111111111111111111111111111</a>');
        echo('</td>');
    }
    echo('</tr></table>');
*/
    ...
    if ($screen_height = Session::get('screen_height')) {
        if ($screen_height > 960) {
            ...
            $graph_array['height'] = max($graph_array['height'], ($screen_height - ($screen_height /2.5)));
        }
    }
...
*/
    ...
    echo('<div style="width: '.$graph_array['width'].';height: 800;margin: auto;"><center>');
    if (Config::get('webui.dynamic_graphs', false) === true) {
        ...
        echo $db_name;
    echo ' <iframe src="http://g.babygoat.biz/grafana/dashboard/script/' . $type . '_' . $subtype . '.js?orgId=1&hostname=' . $device['hostname'] . '&dbname=' . $db_name . '&theme=light&kiosk=tv"  height="' . $graph_array['height'] . '" frameborder="0"></iframe>';
    //    echo generate_lazy_graph_tag($graph_array);
    }
    echo("</center></div>");
