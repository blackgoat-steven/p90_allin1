...
if (!empty($_POST['hostname'])) {
    ...
    if (Auth::user()->hasGlobalRead()) {
        ...
        try {
            ...
            //$command = escapeshellcmd('python /usr/local/librenms/snmp-scan-custom.py '.$hostname.' -t 32 -c '.$community.' -V '.$snmpver.' -u discovery');
            //$output = shell_exec($command);
            //if (strpos($output, 'Runtime:') !== false)
            //{
            //    $discovercommand = escapeshellcmd('php /usr/local/librenms/discovery.php -h new');
            ...
            //    print_message("Add Device Mass complete!");
            //}
            //else{
            //    print_error('SCAN CIDR IP FORMAT ERROR.('.$command.')');
            //}
            print_message("排程已送出，請稍後至<a href='eventlog'>Eventlog</a>確認。");
            print_message("可繼續送出下一個排程。");
...
<form name="form1" method="post" action="" class="form-horizontal" role="form">
    <?php echo csrf_field() ?>
  <div><h2>Add Device Mass</h2></div>
  <div class="alert alert-info">Devices will be checked for Ping/SNMP reachability before being probed.</div>
  <div class="well well-lg">
      <div class="form-group">
          <label for="hostname" class="col-sm-3 control-label">CIDR</label>
          <div class="col-sm-9">
              <input type="text" id="hostname" name="hostname" class="form-control input-sm" placeholder="ex: 192.168.1.0/24">
          </div>
      </div>
      <div class='form-group'>
...
?>
    ...
    <hr>
    <center><button type="button" class="btn btn-default" id="add_device" name="Submit">Add Device</button></center>
  </div>
</form>
  </div>
  <div class="col-sm-3">
...
<script>
    ...
    $(function(){
        $('#add_device').click(function(ev){
            ...
            var message = '<?php print_message("排程已送出，請稍後至<a href=\'devicelog\'>Devicelog</a>確認。"); ?>';
            message += '<?php print_message("可繼續送出下一個排程。"); ?>';
