...
def scan_host(ip,community,snmpversion,user):
    print('running scan_host func')
    hostname = None
    try:
        try:
        ...
        try:
            arguments = ['/usr/bin/env', 'php', 'addhost.php',ip or hostname,community,snmpversion,user]
            print('running scan_host func',arguments)
            print(hostname or ip,community,snmpversion,user)
	    

            print('running scan_host func,args.ping=',args.ping)
            if args.ping:
                arguments.insert(3, args.ping)
            add_output = check_output(arguments)
            print('running scan_host func,add_output=',add_output)
    ...
    print('running scan_host func,final Outcome.UNDEFINED=',Outcome.UNDEFINED)
...
Example: """ + __file__ + """ -P 192.168.0.0/24""")
    ...
    #add jameschen3
    parser.add_argument('-V', '--snmp_version', dest='snmpversion',help="snmp version.")
    parser.add_argument('-c', dest='community', help='SNMP Community String')
    parser.add_argument('-u', dest='user',help='Differentiate users')
        ...
        for network in networks:
            ...
            for ip in ips:
                ...
                if not check_ip_excluded(ip):
                    #print(str(scan_host),str(args.community),str(args.snmpversion))
                    pool.apply_async(scan_host, (str(ip),str(args.community),str(args.snmpversion),str(args.user)), callback=handle_result)
